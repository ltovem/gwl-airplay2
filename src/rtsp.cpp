...
// Small binary-plist responses used by the AirPlay 2 RTSP state machine.
// These are generated by Apple's binary plist format and intentionally kept
// dependency-free so the protocol layer remains portable.
...
std::string setup_event_response_plist() {
    // {'eventPort': 0, 'timingPort': 0}.  The event/timing transports are
    // enabled incrementally; returning the keys here is nevertheless
    // required by modern senders before they issue the stream SETUP.
    static const std::array<unsigned char, 72> bytes = {
        0x62,0x70,0x6c,0x69,0x73,0x74,0x30,0x30,0xd2,0x01,0x02,0x03,0x03,0x59,0x65,
        0x76,0x65,0x6e,0x74,0x50,0x6f,0x72,0x74,0x5a,0x74,0x69,0x6d,0x69,0x6e,0x67,0x50,
        0x6f,0x72,0x74,0x10,0x00,0x08,0x0d,0x17,0x22,0x00,0x00,0x00,0x00,0x00,0x00,0x01,
...
RtspResponse RtspSession::setup(const RtspRequest& request) {
    ...
    if (!setup_info_complete_ && is_binary_plist(request.body)) {
        ...
        auto response = base_response(request, 200, "OK");
        response.body = setup_event_response_plist();

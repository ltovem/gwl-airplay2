...
bool AirPlayHkpPairing::handle_pair_setup(const std::vector<std::uint8_t>&,
                                          std::vector<std::uint8_t>& response,
                                          std::string& error) {
    response.clear(); error = "OpenSSL crypto backend is required for AirPlay HKP pairing"; return false;
}
bool AirPlayHkpPairing::handle_pair_verify(const std::vector<std::uint8_t>&,
                                           std::vector<std::uint8_t>& response,
                                           std::string& error) {
    response.clear(); error = "OpenSSL crypto backend is required for AirPlay HKP pairing"; return false;
}
...
bool random32(std::array<std::uint8_t, 32>& out) {
    return RAND_bytes(out.data(), static_cast<int>(out.size())) == 1;
}

bool aes128_ctr(const std::vector<std::uint8_t>& input,
    ...
    bool ok = EVP_EncryptInit_ex(ctx, EVP_aes_128_ctr(), nullptr, key.data(), iv.data()) == 1 &&
              EVP_EncryptUpdate(ctx, output.data(), &n1, input.data(), static_cast<int>(input.size())) == 1 &&
              EVP_EncryptFinal_ex(ctx, output.data() + n1, &n2) == 1;
...
bool sign_ed25519(EVP_PKEY* key, const std::vector<std::uint8_t>& message,
                  std::vector<std::uint8_t>& signature) {
    EVP_MD_CTX* ctx = EVP_MD_CTX_new();
    if (!ctx) return false;
    std::size_t len = 64;
    signature.resize(len);
    bool ok = EVP_DigestSignInit(ctx, nullptr, nullptr, nullptr, key) == 1 &&
              EVP_DigestSign(ctx, signature.data(), &len, message.data(), message.size()) == 1;
    EVP_MD_CTX_free(ctx);
...
bool verify_ed25519(EVP_PKEY* key, const std::vector<std::uint8_t>& message,
                    const std::vector<std::uint8_t>& signature) {
    EVP_MD_CTX* ctx = EVP_MD_CTX_new();
    if (!ctx) return false;
    bool ok = EVP_DigestVerifyInit(ctx, nullptr, nullptr, nullptr, key) == 1 &&
              EVP_DigestVerify(ctx, signature.data(), signature.size(), message.data(), message.size()) == 1;
    EVP_MD_CTX_free(ctx);
    return ok;
}

bool x25519_shared(EVP_PKEY* private_key, const std::vector<std::uint8_t>& peer_public,
    ...
    bool ok = ctx && EVP_PKEY_derive_init(ctx) == 1 && EVP_PKEY_derive_set_peer(ctx, peer) == 1 &&
              EVP_PKEY_derive(ctx, shared.data(), &len) == 1;
...
bool AirPlayHkpPairing::handle_pair_setup(const std::vector<std::uint8_t>& request,
                                          std::vector<std::uint8_t>& response,
                                          std::string& error) {
    response.clear(); error.clear();
    // iOS/macOS HKP pairing sends exactly 32 opaque bytes. The receiver returns
    // its Ed25519 public key; the request bytes themselves are not TLV8.
    ...
    if (!impl_->ed25519) {
        ...
        if (!impl_->ed25519 || !raw_public(impl_->ed25519, impl_->ed_public)) {
            error = "failed to generate Ed25519 identity key"; return false;
        }
    }
...
bool AirPlayHkpPairing::handle_pair_verify(const std::vector<std::uint8_t>& request,
                                           std::vector<std::uint8_t>& response,
                                           std::string& error) {
    response.clear(); error.clear();
    ...
    if (state == 1) {
        ...
        if (!impl_->ed25519 || impl_->client_curve_public.size() != 32 || impl_->client_ed_public.size() != 32) {
            error = "pair-setup identity is not established"; return false;
        }
        ...
        if (!raw_public(impl_->x25519, server_curve) || !x25519_shared(impl_->x25519, impl_->client_curve_public, impl_->shared)) {
            error = "Curve25519 key agreement failed"; return false;
        }
        ...
        if (!aes128_ctr(signature, key, iv, encrypted)) { error = "AES-CTR encryption failed"; return false; }
    ...
    if (state == 0) {
        if (impl_->shared.empty() || impl_->client_ed_public.size() != 32) { error = "pair-verify state is not established"; return false; }
        ...
        std::vector<std::uint8_t> iv(iv_hash.begin(), iv_hash.begin() + 16);
        std::vector<std::uint8_t> signature;
        if (!aes128_ctr(encrypted, key, iv, signature) || signature.size() != 64) { error = "AES-CTR decryption failed"; return false; }

...
bool aes128_ctr(const std::vector<std::uint8_t>& input, const std::vector<std::uint8_t>& key, const std::vector<std::uint8_t>& iv, std::vector<std::uint8_t>& output, std::size_t discard_prefix = 0) {
    ...
    if (!ctx) return false;
    std::vector<std::uint8_t> prefix(discard_prefix, 0);
    ...
    if (ok && discard_prefix) ok = EVP_EncryptUpdate(ctx, prefix.data(), &dummy, prefix.data(), static_cast<int>(prefix.size())) == 1;
...
bool AirPlayHkpPairing::handle_pair_verify(const std::vector<std::uint8_t>& request, std::vector<std::uint8_t>& response, std::string& error) {
    ...
    if (state == 1) {
        // M1 = 01 00 00 00 || client Curve25519 public key || client Ed25519 public key.
        ...
        // AirPlay signs ServerCurve || ClientCurve. The AES-CTR stream first
        // consumes the 32-byte client challenge, then encrypts the 64-byte signature.
        ...
        auto iv_hash = sha512("Pair-Verify-AES-IV", impl_->shared);
    ...
    if (state == 0) {
        if (!impl_->shared.empty() && impl_->client_ed_public.size() == 32) {
            // M3 = 00 00 00 00 || encrypted 64-byte client signature.

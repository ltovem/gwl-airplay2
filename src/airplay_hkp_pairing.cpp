...
bool aes128_ctr(const std::vector<std::uint8_t>& input, const std::vector<std::uint8_t>& key, const std::vector<std::uint8_t>& iv, std::vector<std::uint8_t>& output) {
...
bool AirPlayHkpPairing::handle_pair_verify(const std::vector<std::uint8_t>& request, std::vector<std::uint8_t>& response, std::string& error) {
    ...
    if (state == 1) {
        ...
        // Encrypt exactly the 64-byte Ed25519 signature from the beginning of
        // the AirPlay Pair-Verify AES-CTR stream. There is no 32-byte discard.
